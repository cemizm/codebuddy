...
@patch('pycodebuddy.transaction_handler.subprocess.run')
@patch('pycodebuddy.transaction_handler.TransactionHandler._has_changes', return_value=True)
def test_commit_success(mock_has_changes, mock_run, handler):
    ...
    mock_run.assert_any_call(
        ['git', 'commit', '-m', 'Test commit'], check=True, capture_output=True, text=True)


@patch('pycodebuddy.transaction_handler.subprocess.run')
@patch('pycodebuddy.transaction_handler.TransactionHandler._has_changes', return_value=False)
...
def test_rollback_success(mock_run, handler):
    handler.rollback()
    mock_run.assert_any_call(
        ["git", "reset", "--hard", "HEAD"], check=True, capture_output=True, text=True)
    mock_run.assert_any_call(["git", "clean", "-fd"],
